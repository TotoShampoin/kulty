...
function placePlaylist(playlist, type) {
    ...
    $("#category-container").append(`
        <div class="category" id="${type[0]}${id}" data-type="${type}" data-play="${id}">
            <div class="category-title hasImg">
                <h2>${name}</h2>
            </div>
        </div>`);
    $(`#${type[0]}${id} .category-title`).css("background-image", `url(https://cdn.kulty.app/${type}/${photo})`);
    if(photo == undefined) {
        $(`#${type[0]}${id} .category-title`).css("background-image", `url(https://cdn.kulty.app/${type}/noimg.jpg)`);
    }
}
    ...
    $(`#${type[0]}${playlist}`).append(`
        <div class="expo cate-expo hasImg" id="${type[0]}${playlist}-e${expoID}" data-expo="${expoID}">
            ...
            <div class="expo-buttons">
                ...
                <a href="https://redirect.kulty.app/loc.php?id=${expoID}" class="link loc"><img src="/assets/icons/icone_filaire_lieu_blanc.svg" alt="Y aller" class="icon-img"></a>
...
function placeExpoFeed(expo) {
    ...
    <div class="top-expo-micro hasImg" id="top-micro-e${expoID}" data-expo="${expoID}">
        <div class="top-expo-micro-buttons">
            <div class="buttons">
                <a href="https://redirect.kulty.app/loc.php?id=${expoID}" class="link loc"><img src="/assets/icons/icone_plein_lieu_blanc.svg" alt="Y aller" class="icon-img">Y aller</a>
...
function setClick(e) {
    ...
    e.preventDefault();
    e.stopPropagation();
    if( ! ($(e.target).hasClass("link") || $(e.target).hasClass("expo-buttons") || $(e.target).hasClass("icon-img") || $(e.target).hasClass("friend-close") || $(e.target).parent().hasClass("friend-close")) ) {
        const $this = $(this);
